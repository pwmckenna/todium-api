...
var addFirebaseTracker = function(firebase, provider, id, info_hash, src) {
...
var addLinkRequest = function(firebase, provider, id, src) {
    ...
    if(src.indexOf(MAGNET_LINK_IDENTIFIER) === 0) {
        ...
        var name = addFirebaseTracker(firebase, provider, id, info_hash, src);
        ...
        request.get({
            ...
            if(error || typeof info_hash !== 'string' || info_hash.length !== 40) {
                ...
                var name = addFirebaseTracker(firebase, provider, id, info_hash, src);
                var tracker = 'http://tracker.todium.com/' + name + '/announce';
                var torrent = 'http://torrent.todium.com/' +
                    '?tracker=' + tracker +
                    '&torrent=' + src;
                ret.resolve(torrent);
            }
...
http.createServer(function(req, res) {
    ...
    authenticationRequest.then(function(info) {
        ...
        var id = info.auth.id;
        addLinkRequest(firebase, provider, id, src).then(function(link) {
