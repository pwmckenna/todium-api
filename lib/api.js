...
var addFirebaseTorrent = function(firebase, provider, id, tracker, torrent) {
...
var addLinkRequest = function(firebase, provider, id, src, labels) {
    ...
    if(src.indexOf(MAGNET_LINK_IDENTIFIER) === 0) {
        ...
        var tracker = addFirebaseTracker(firebase, provider, id, info_hash, src, labels);
        ...
        request.get({
            ...
            if(error || typeof info_hash !== 'string' || info_hash.length !== 40) {
                ...
                var tracker = addFirebaseTracker(firebase, provider, id, info_hash, src, labels);
                var url = 'http://tracker.todium.com/' + tracker.name() + '/announce';
                var trackable = addFirebaseTorrent(firebase, provider, id, url, src);
                tracker.child('trackable').set(trackable);
                ret.resolve(trackable);
            }
        });
